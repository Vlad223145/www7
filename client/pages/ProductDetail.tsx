...
const products: Record<string, Product> = {
  "sparklesoft-hand-wash": {
    ...
    description: "Gentle yet effective hand wash with sparkling clean formula that removes dirt and germs while keeping hands soft and moisturized.",
  ...
  "soothesuds-hand-wash": {
    ...
    description: "Luxurious soothing hand wash enriched with calming botanicals to cleanse and comfort sensitive skin with every use.",
  ...
  "vitalguard-hand-wash": {
    ...
    description: "Advanced protection hand wash with vital nutrients and antioxidants that defend against germs while nourishing skin cells.",
  ...
  "freshcrisp-hand-wash": {
    ...
    description: "Invigorating hand wash with crisp citrus blend that energizes while providing thorough cleansing and refreshing sensation.",
  ...
  "velvetclean-hand-wash": {
    ...
    description: "Luxurious velvet-soft hand wash that transforms rough hands into silky smooth perfection with premium moisturizing ingredients.",
  ...
  "refreshrinse-hand-wash": {
    ...
    description: "Cooling refresh hand wash with mint and eucalyptus that provides instant refreshment and thorough cleansing experience.",
      ...
      <div style={{ backgroundColor: "rgb(255, 255, 255)", minHeight: "100vh" }}>
        ...
        <div role="banner" style={{ paddingBottom: "15px", paddingTop: "15px", position: "relative", zIndex: "1000" }}>
          <div style={{ marginLeft: "auto", marginRight: "auto", maxWidth: "100%", paddingLeft: "150px", paddingRight: "150px" }}>
            <div style={{ alignItems: "center", display: "grid", gap: "40px", gridTemplate: "auto / 0.5fr 1.5fr 0.5fr", justifyItems: "center" }}>
              <Link to="/" style={{ gridArea: "span 1 / span 1", justifySelf: "start", textDecoration: "none" }}>
                <RawImg
                  loading="lazy"
                  alt="handly logo"
                  image="https://cdn.prod.website-files.com/65f9834ec170577cb6334d0d/65fbc8f1bc1d9dd0f7d9e8fb_handly.svg"
                  css={{ display: "inline-block", maxWidth: "100%", verticalAlign: "middle" }}
                />
              </Link>
              
              <nav style={{ justifySelf: "start" }}>
                <Link to="/" style={{ padding: "10px 20px", textDecoration: "none", color: "rgb(0, 0, 0)" }}>Home</Link>
                <Link to="/about" style={{ padding: "10px 20px", textDecoration: "none", color: "rgb(0, 0, 0)" }}>About</Link>
                <Link to="/shop" style={{ padding: "10px 20px", textDecoration: "none", color: "rgb(0, 0, 0)", fontWeight: "bold" }}>Shop</Link>
                <Link to="/contact" style={{ padding: "10px 20px", textDecoration: "none", color: "rgb(0, 0, 0)" }}>Contact</Link>
              </nav>

              <div style={{ justifySelf: "end" }}>
                <Link to="/" style={{ padding: "10px", textDecoration: "none", color: "rgb(0, 0, 0)" }}>← Back</Link>
              </div>
            </div>
          </div>
        </div>
        ...
        <div style={{ padding: "100px 150px" }}>
          <div style={{ display: "grid", gridTemplate: "auto / 1fr 1fr", gap: "80px", alignItems: "start" }}>
            {/* Product Images */}
            <div>
              <div style={{ position: "relative", borderRadius: "20px", overflow: "hidden", marginBottom: "20px" }}>
                <RawImg
                  ...
                  loading="eager"
                  image={product.image}
                  css={{ width: "100%", height: "600px", objectFit: "cover", borderRadius: "20px" }}
                />
              </div>
              <div style={{ display: "grid", gridTemplate: "auto / 1fr 1fr", gap: "20px" }}>
                <RawImg
                  ...
                  loading="lazy"
                  image={product.image}
                  css={{ width: "100%", height: "200px", objectFit: "cover", borderRadius: "10px", cursor: "pointer" }}
                />
                <RawImg
                  alt={product.name}
                  loading="lazy"
            ...
            {/* Product Info */}
            <div>
              <h1 style={{ fontSize: "48px", fontFamily: "Generalsans, sans-serif", marginBottom: "20px", lineHeight: "1.2" }}>
                {product.name}
              </h1>
              
              <div style={{ display: "flex", alignItems: "center", gap: "20px", marginBottom: "30px" }}>
                <div style={{ fontSize: "32px", fontWeight: "bold", fontFamily: "Generalsans, sans-serif" }}>
                  {product.price}
                </div>
                {product.originalPrice && (
                  <div style={{ fontSize: "24px", textDecoration: "line-through", opacity: "0.5" }}>
                    {product.originalPrice}
                  </div>
                )}
              </div>

              <p style={{ fontSize: "18px", lineHeight: "1.6", marginBottom: "40px", color: "rgb(100, 100, 100)" }}>
                {product.description}
              </p>
                ...
                <div style={{ display: "grid", gap: "10px" }}>
                  {product.features.map((feature, index) => (
                    <div key={index} style={{ display: "flex", alignItems: "center", gap: "10px" }}>
                      <div style={{ width: "8px", height: "8px", backgroundColor: "rgb(0, 0, 0)", borderRadius: "50%" }}></div>
                      <span>{feature}</span>
                    </div>
                  ))}
                </div>
                ...
                <div>
                  ...
                  <div style={{ display: "grid", gap: "8px" }}>
                    {product.ingredients.map((ingredient, index) => (
                      <div key={index} style={{ display: "flex", alignItems: "center", gap: "10px" }}>
                        <div style={{ width: "6px", height: "6px", backgroundColor: "rgb(100, 100, 100)", borderRadius: "50%" }}></div>
                        <span style={{ color: "rgb(100, 100, 100)" }}>{ingredient}</span>
                      </div>
                    ))}
                  </div>
                </div>
